...
Waveform::realseq_t Gen::ImpactTransform::waveform(int iwire) const
{
  ...
  for (int imp = min_impact; imp <= max_impact; ++imp) {
      ...
      if (!id) {
          ...
          //std::cerr << "ImpactTransform: no data for absolute impact number: " << imp << std::endl;
          continue;
      }
      ...
      if (charge_spectrum.empty()) {
          // should not happen
          std::cerr << "ImpactTransform: no charge for absolute impact number: " << imp << std::endl;
            continue;
        }
        if (weightcharge_spectrum.empty()) {
            // weight == 0, should not happen
            std::cerr << "ImpactTransform: no weight charge for absolute impact number: " << imp << std::endl;
            continue;
        }
	...
	//	std::cout << "IZ: " << " imp=" << imp << " imp_pos=" << imp_pos << " wire_pos=" << wire_pos << " rel_imp_pos=" << rel_imp_pos << std::endl;
        ...
        if (share) {            // fixme: make a configurable option
            ...
            if (!two_ir.first || !two_ir.second) {
                //std::cerr << "ImpactTransform: no impact response for absolute impact number: " << imp << std::endl;
                continue;
            }
        ...
        ++nfound;
        // std::cerr << "ImpactTransform: found:"<<nfound<<" for absolute impact number " << imp
        //           << " csize=" << csize << " rsize=" << rsize << " rebin=" << rebinfactor
        //           << std::endl;

        Waveform::increase(total_spectrum, conv_spectrum);
    }
    //std::cerr << "ImpactTransform: found " << nfound << " in abs impact: ["  << min_impact << ","<< max_impact << "]\n";
